...
st.markdown("<h2 style='color: green;'>Customer information management</h2>", unsafe_allow_html=True)
...
if st.button("add"):
    db.modify_customer('add',Phone , '', Name)
...
with col3:
    Phone = st.text_input("customer's phone")

if st.button("remove"):
    db.modify_customer('remove', Phone)
...
with col7:
    updatedPhone = st.text_input("updated phone")

if st.button("update"):
    db.modify_customer('update', Phone, updatedPhone, Name)

a = db.show_tables('Customer')


st.markdown("---")

if st.button('Show Customer Table'):
    df = db.query("SELECT * FROM Customer")
    st.write(df)